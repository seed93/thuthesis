...
\begin{figure}[htbp]
  \centering
  \subcaptionbox{}{\includegraphics[height=5.5cm]{bkfar}\hspace{1cm}}
  \subcaptionbox{}{\includegraphics[height=5.5cm]{bkfarcut}}
...
\begin{figure}[htbp]
  \centering
  \subcaptionbox{原图}{\includegraphics[width=0.4\textwidth]{bkfootorg}\hspace{1cm}}
  \subcaptionbox{背景}{\includegraphics[width=0.4\textwidth]{bkfootbk}}\\
  \subcaptionbox{原图HSV}{\includegraphics[width=0.4\textwidth]{bkfootorghsv}\hspace{1cm}}
  \subcaptionbox{背景HSV}{\includegraphics[width=0.4\textwidth]{bkfootbkhsv}}\\
  \subcaptionbox{做差}{\includegraphics[width=0.4\textwidth]{bkfootdiff}}\\
  \caption{去除阴影}
\end{figure}
...
此处需要强调一下，Poppe等人对于区域数量的选取是$5\times 6=30$，由于每张图片人物大小不一、长宽比例不固定，因此每个子区域的长宽比也不固定，这会导致一些问题，比如在不同长宽比的图片中，头部在细长的图片中会出现在多个子区域中，而较宽的图片中则只有一个区域有头部，图片本身的比例信息已经丢失，为了解决这个问题，本文在求取HOG特征之前先将图片填充到3:2 的比例，这样求取的特征就不会变形，虽然有较多区域是无用信息，占用了维数，但是经过测试，这并不会增加额外的存储占用，因为数据本身是稀疏的。
...
\begin{figure}[htbp]
  ...
  \subcaptionbox{HOG2}{\includegraphics[width=0.2\textwidth]{HOG3}}
  \caption{HOG特征}
\end{figure}
